		...
		virtual void GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const override
		{
			...
			for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
			{
				if (VisibilityMap & (1 << ViewIndex))
				{
					...
					PDI->DrawLine(LocalToWorld.GetOrigin(), LocalToWorld.TransformPosition(FBarragePrimitive::UpConvertFloatVector(BarragePosition)), FColor(255, 19, 240), SDPG_World, LineThickness * 1.2f);
					// Draw a ghost boxy thing at the position of the barrage body
					DrawOrientedWireBox(PDI, LocalToWorld.TransformPosition(FBarragePrimitive::UpConvertFloatVector(BarragePosition)), LocalToWorld.GetScaledAxis(EAxis::X), LocalToWorld.GetScaledAxis(EAxis::Y), LocalToWorld.GetScaledAxis(EAxis::Z), BoxExtents, FColor(255, 19, 240), SDPG_World, LineThickness * 0.8f);
				}
			}
		}
